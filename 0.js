...
// 设置资源保存路径
files.createWithDirs("/sdcard/学习小助手/");
// 调整音量
if (yl_on) {
  fInfo("设置媒体音量");
...
function do_duizhan1(renshu) {
  ...
  while (true) {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (!que_txt) {
        images.save(img, '/sdcard/学习小助手/' + renshu + '-' + num + '.png', 'png', 50);
        images.save(que_img, '/sdcard/学习小助手/' + renshu + '-' + num + '-q.png', 'png', 50);
        fError("未识别出题目，图片保存至‘/sdcard/学习小助手/’");
    ...
    // log(allx_txt);
    if (!allx_txt) {
      images.save(img, '/sdcard/学习小助手/' + renshu + '-' + num + '-a.png', 'png', 50);
      log("识别不出选项文本，图片保存至‘/sdcard/学习小助手/’");
      err_flag = false;
      sleep(200);
      continue;
    }
...
function send_email(email) {
  ...
  data.putExtra(Intent.EXTRA_SUBJECT, "学习小助手：" + name);
...
function fInit() {
  ...
  var w = floaty.rawWindow(
    <card cardCornerRadius='8dp' alpha="0.8">
      <vertical>
        ...
        <text id='version' textColor="#FFFFFF" textSize="18dip">学习小助手+</text>
  ...
  ui.run(function () {
    ...
    w.version.setText("学习小助手+" + newest_version);
  });
  w.setSize(720, -2);
  w.setPosition(10, 10);
